...
url_base = "https://projects.fivethirtyeight.com/polls-page/data/"
polls = read.csv(paste0(url_base, "president_polls.csv")) %>% 
...
yfloor = floor(10 * lower) / 10
yceil  = ceiling(10 * upper) / 10
...
sampler <- function(y, X, N = 1000, b = NULL, s = NULL) {
    k <- ncol(X)
    n <- nrow(X)
    b <- "if"(is.null(b), rnorm(k), b)
    s <- rgamma(1, shape = 1)
    B <- matrix(nrow = k, ncol = N)
    S <- numeric(N)
    for ( i in 1:N ) {
        ## Draw beta
        z <- rnorm(k, mean = b)
